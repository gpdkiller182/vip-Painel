...
if login == 1:
    ...
    if input(f"{C}[{G}+{C}]PASSWORD:  ").strip() == snh:
        ...
        exit()
    print("\n ")
if user == 'YATO' or user == 'GOD KILLER':
    kinymode=1
    kiny=1
...
while(Sair == False):
    ...
    for letter in "P A I N E L V I P":
    ...
    tools.clear()
    time.sleep(2)
    print(f"{C}[{G}Dev's{C}] Coded By: {CY} GOD KILLER {CO} and {CY} YATO {CO} in 07/02/2021")
    print()
    print(f'{C}{G}{result}{C}')    
    ...
    {C}{G}[3]{C} GOD KILLER-SITE-INFOGA
